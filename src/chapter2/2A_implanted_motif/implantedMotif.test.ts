import fs from "fs";
import implantedMotif from "./implantedMotif";

describe("2A: Implanted Motif Problem", () => {
  test("Rosalind Sample dataset", () => {
    const dna = ["ATTTGGC", "TGCCTTA", "CGGTATC", "GAAAATT"];
    const k = 3;
    const d = 1;
    const ans = implantedMotif(dna, k, d);
    const expAns = "ATA ATT GTT TTT";
    const test = expAns.split(" ");
    expect(ans.size).toBe(test.length);
    // for each sub of exp ans remove from ans
    test.forEach((s) => ans.delete(s));
    expect(ans.size).toBe(0);
  });
  ...
  test("Rosalind Submit dataset", () => {
    const dna = [
      "AAAGCAACCGCGCTGAGAGTGTACT",
      "TCGCCATACTCGGACCTGTACGATG",
      "GGTTGAGTCGGTACTTCCGAACGTA",
      "ATACTCGGGTAGTACGAGGTTGTAG",
      "TGAAAGCTAACGTAACCGCAATACT",
      "GAGTTGTACTGGCTTAACTATTTCT",
      "GCATCATTACCTACTTTAATTTGCG",
      "ATACCGTTGCTAACAGGTTTATACT",
      "TGGCAGCTGTGCCTCCTACTCTACG",
      "TCGTTAGATCATACTGGTGCCCCGA",
    ];
    const k = 5;
    const d = 1;
    const ans = implantedMotif(dna, k, d);

    // const write = [...ans.keys()].toString().replace(/,/g, " ");
    // fs.writeFileSync(`./src/chapter2/2A_implanted_motif/ans/ans.txt`, write);

    const expAns = "ATACT CTACT GTACT TTACT GTACC";
    const test = expAns.split(" ");
    expect(ans.size).toBe(test.length);
    // for each sub of exp ans remove from ans
    test.forEach((s) => ans.delete(s));
